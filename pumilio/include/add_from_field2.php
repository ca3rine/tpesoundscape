...
?>
	...
	<div class="container">
		...
		<div class="span-24 last">
			...
			$recursive_files_to_process = array();
			$lsfiles = shell_exec('ls -R -1 '. $localdir);
			$tmp_lines = explode("\n", $lsfiles);
			foreach ($tmp_lines as $tmp_line) {
				if (empty($tmp_line)) {
					$parent_dir = "";
				}
				else if (preg_match('/\:$/', $tmp_line)) {
					$parent_dir = trim($tmp_line, ":");
				}
				else if (($parent_dir != '')&&(preg_match('/\.wav$/', $tmp_line)||preg_match('/\.WAV/', $tmp_line))) {
					$recursive_files_to_process[] = $parent_dir . "/" . $tmp_line;
				}
			}

			if ($local == 1){
				$dir = $localdir;
				if (!empty($recursive_files_to_process)) {
					$files_to_process = $recursive_files_to_process;
					$files_to_process_counter = count($recursive_files_to_process);
					echo "<xmp>"; var_dump($files_to_process); echo "</xmp>";
				}
				else {

...
#		$random_value = md5(microtime(true));
		...
		if ($file_format == "flac") {
			...
			if ($SamplingRate!=44100 || $SamplingRate!=22050 || $SamplingRate!=11025) {
				...
				if ($retval!=0) {
					exit("<p class=\"error\">There was a problem with SoX... (in flac)</div>");
					}
			...
			if ($retval!=0) {
				exit("<p class=\"error\">There was a problem with SoX... (file is not mp3 and not flac)</div>");
				}
			}
		...
		if ($file_format == "flac") {
			...
			if ($SamplingRate != 44100 || $SamplingRate != 22050 || $SamplingRate != 11025) {
				...
				if ($retval != 0) {
					...
					exit("<p class=\"error\">There was a problem with SoX... (in ogg flac)</div>");
					}
			...
			if ($retval != 0) {
				save_log($connection, $SoundID, "80", "SoX had a problem with sounds/sounds/$ColID/$DirID/$filename." . $lastline);
				exit("<p class=\"error\">There was a problem with SoX... (in ogg wav)</div>");
				}
			}
